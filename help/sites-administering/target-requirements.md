...
## Register with Adobe Target {#registering-with-adobe-target}
...
>[!NOTE]
>
>The Target team must enable your account to use the integration.
>
>If it is not the case, contact [Adobe Customer Care](https://experienceleague.adobe.com/en/docs/target/using/cmp-resources-and-contact-information).
...
1. Select the Enabled option, then click **OK**.
   ...
   >
   >If you try to test the connection with `tnt:///`, it shows an error which is the expected behavior. The reason is because the URI is for internal use only. Do not use with **Test Connection**.

## Secure the activity settings node {#securing-the-activity-settings-node}

Secure the activity settings node **cq:ActivitySettings** on the publishing instance so that it is inaccessible to normal users. The activity settings node should only be accessible to the service handling the activity synchronization to Adobe Target.
...
## Configure the AEM Link Externalizer {#configuring-the-aem-link-externalizer}
