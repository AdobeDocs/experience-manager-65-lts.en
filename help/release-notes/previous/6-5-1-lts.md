...
>[!IMPORTANT]
>
> **Mandatory Hotfix** – To avoid SNFE (SegmentNotFoundException) issues with offline compaction when installing Service Pack 1, install the hotfix described in [Known issues – Repository corruption during online compaction](#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146).
...
* Oracle Java&trade; 17/21 (64 bit) (See more details in the [Java Support](#java-support) section.

**Application Server Support Updates**

* Support for JBoss&reg; EAP 8 has been added.
...
AEM Forms on JBoss&reg; EAP 8 uses Elytron for managing secure credentials. Customers must configure an Elytron-based Credential Store to ensure successful server startup and secure database authentication.
...
>[!NOTE]
>
> (Forms only) Oracle Java&trade; 8/11 (64 bit) required for one-time installation.
...
* **FORMS-24688:** When setting up JBoss EAP 8.0 (AEM Forms 6.5.1 LTS) on Linux, scripts created or edited in a Windows may contain CRLF line endings, causing /bin/sh^M: bad interpreter or $'\r': command not found errors. Ensure that affected files use LF line endings. This can be accomplished using tools such as dos2unix or any equivalent method.

* **FORMS-24689:** In Turnkey mode, the database port update is not applied correctly during fresh installation or upgrade. In fresh installation mode, users can only select from the ports displayed on the screen. In Upgrade mode, the new database port must be manually updated in lc_turnkey.xml before proceeding with the upgrade.

* **FORMS-24687:** In AEM Forms 6.5 LTS cluster deployments on JBoss EAP 8, the domain/configuration/domain_oracle.xml, domain_mysql.xml, and domain_mssql.xml files may contain a duplicate `<security>` tag, resulting in invalid XML that prevents the Domain Controller from starting.
...
Users experienced repository corruption during online compaction if offline compaction previously ran on the JCR repository. A `SegmentNotFoundException` (SNFE) can occur in this scenario. It can lead to repository corruption.

To resolve the issue, install the hotfix via Package Manager from [Software Distribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-65385-1.0.zip). Because the hotfix includes a low-level `oak-segment-tar` bundle, the instance restarts after installation. 

Plan for downtime of an instance when applying it. For offline compaction, use the corresponding [Oak-run jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar), also available on Software Distribution.
...
### Missing `com.adobe.granite.apicontroller` bundle in AEM 6.5 LTS SP1 (GRANITE-67639) {#missing-apicontroller-bundle-granite-67639}

The `com.adobe.granite.apicontroller` bundle is missing in AEM 6.5 LTS SP1. This bundle controls how OSGi bundles are resolved and can prevent bundles from resolving to other bundles, which is useful for limiting exposed APIs.

Install the hotfix from [Software Distribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-67639-1.0.zip) to use this functionality.
...
* [List of OSGi bundles included in Experience Manager 6.5 LTS, Service Pack 1](/help/release-notes/assets/65lts_sp1_bundles.zip) <!-- UPDATE FOR EACH NEW RELEASE -->
* [List of Content Packages included in Experience Manager 6.5 LTS, Service Pack 1](/help/release-notes/assets/65lts_sp1_packages.zip) <!-- UPDATE FOR EACH NEW RELEASE -->
