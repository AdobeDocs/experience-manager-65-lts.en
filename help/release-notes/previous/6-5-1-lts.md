...
* **FORMS-24688:** WWhen setting up JBoss EAP 8.0 (AEM Forms 6.5.1 LTS) on Linux, scripts modified on Windows may contain CRLF line endings, causing /bin/sh^M: bad interpreter or $'\r': command not found errors. Convert the affected .sh files to Unix (LF) format using dos2unix (for example, find . -type f -name "*.sh" -exec dos2unix {} +) to resolve the issue.

* **FORMS-24689:** In Turnkey Upgrade Mode, updates to the database port are not picked from the UI and must be modified manually in the lc_turnkey.xml configuration file. If not updated correctly, the system may continue to use the old port value.

* **FORMS-24687:** In AEM Forms 6.5.1 LTS cluster deployments on JBoss EAP 8, `domain/configuration/domain_oracle.xml` may contain a duplicate `<security>` tag, resulting in invalid XML that prevents the Domain Controller from starting.
