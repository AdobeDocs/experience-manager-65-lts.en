...
* **FORMS-24688:** When setting up JBoss EAP 8.0 on Linux, shell scripts modified on Windows may contain CRLF line endings, causing `/bin/sh^M: bad interpreter` or $`\r`: command not found errors. Convert affected files to LF line endings using dos2unix to resolve this.

* **FORMS-24689:** In Turnkey Upgrade Mode, updating the database port in `lc_turnkey.xml` may not be applied correctly during upgrade, causing the process to reference the old port value.
