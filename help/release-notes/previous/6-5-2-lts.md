...
* Added a Configuration Manager (LCM) Express Mode prompt for the AEM administrator credentials during configuration, replacing built-in default credentials. This enhancement is available through the 6.5 LTS SP2 JEE hotfix. (FORMS-26704) 
...
**The following issues are fixed in the AEM Forms 6.5 LTS Service Pack 2 JEE hotfix (build 6.6.0-0006). To apply them, [download and install the hotfix](/help/forms/using/adobe-experience-manager-forms-65-lts-hotfixes.md).**

* After Apache Shiro was updated to version 2.1.0, AEM Forms on JEE failed to bootstrap with a `NoClassDefFoundError` for the Shiro security manager. Bootstrapping now completes successfully. (FORMS-26818)
* AEM Forms on JEE failed with a "no class found" error for `org.owasp.esapi.reference.JavaLogFactory`. The missing class is now resolved. (FORMS-26819)
* After upgrading to AEM Forms 6.5 LTS, TaskManager endpoints were removed. The endpoints are now retained. (FORMS-26584, FORMS-26589)
* On JEE, the Configuration Manager MergeEars step failed with a DOCTYPE declaration error (`ALC-LCM-010-200`) because of the secure XML builder. MergeEars now completes. (FORMS-26569)
* Application-level logs were missing on IBM&reg; WebSphere&reg; Liberty deployments. Application-level logging is now restored. (FORMS-25063)
* On JBoss&reg;, email failed with "IMAPProvider not a subtype." Email now works on JBoss&reg;. (FORMS-24892)
* On WebSphere&reg; Liberty Profile (WLP), email failed with "Could not convert socket to TLS." Email over TLS now works on WLP. (FORMS-24692)
* Updated the Gibson library to version 6.0.29665850. (FORMS-26688)
* Backported SAML assertion validation improvements. (FORMS-25222)
* Updated Apache Log4j to version 2.25.5. (FORMS-26733, FORMS-26734)
* This hotfix also includes security fixes.
