...
>[!NOTE]
>
>The ImageMagick `policy.xml` and `configure.xml` files are available at `/usr/lib64/ImageMagick-&#42;/config/` instead of `/etc/ImageMagick/`. See the ImageMagick documentation (`https://www.imagemagick.org/script/resources.php` website) for location of the configuration files.
